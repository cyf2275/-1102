...
FULL_PREFIX=phase_pred_e63_ens_e28_e62_w055_e123_donly
SAVE=results/fpp960_e126_pipfull_e63_e123_donly_depthdiff_ch24_e16
MASTER=results/remote_logs/e126_pipfull_donly_master.log

echo "START E126 PIP-Full ablation E63 + E123 D_c-only diffusion $(date '+%F %T')" | tee "$MASTER"
...
"$PY" precompute_coarse_condition_cache.py \
  ...
  --num_workers 8 \
  --require_cache \
  2>&1 | tee results/remote_logs/e126_precompute_donly.log
...
"$PY" train_pip_lite.py \
  ...
  --skip_final_test \
  2>&1 | tee results/remote_logs/e126_pipfull_donly_train.log

echo "DONE E126 PIP-Full ablation E63 + E123 D_c-only diffusion $(date '+%F %T')" | tee -a "$MASTER"
