...
SAVE=results/fpp960_e56_uph_priorres_local_ch24_e30
PREFIX=phase_pred_e56_uph_priorres_local_ddim20_ftp01
DEPTH_OUT=results/fpp960_e56_uph_priorres_local_calibrated_depth

echo "START E56 local UPH-residual diffusion $(date '+%F %T')" | tee results/remote_logs/e56_priorres_local_master.log
...
"$PY" train_fpp_phase_diffusion.py \
  ...
  --uph_select_weight 30.0 \
  --calib_depth_weight 0.10 \
  --calib_depth_summary "$CALIB" \
  --calib_depth_source gt_raw \
  --calib_depth_scale 80.0 \
  ...
  --save_every 1 \
  --seed 44 \
  2>&1 | tee results/remote_logs/fpp960_e56_uph_priorres_local_ch24_e30.log

echo "PRECOMPUTE E56 $(date '+%F %T')" | tee -a results/remote_logs/e56_priorres_local_master.log
"$PY" precompute_fpp_phase_diffusion_predictions.py \
  ...
  --sample_start_from ftp \
  --sample_start_ratio 0.1 \
  2>&1 | tee results/remote_logs/e56_priorres_local_precompute.log

echo "CALIBRATED DEPTH E56 $(date '+%F %T')" | tee -a results/remote_logs/e56_priorres_local_master.log
"$PY" eval_phase_calibrated_depth.py \
  ...
  --uph_prior_summary "$PRIOR" \
  2>&1 | tee results/remote_logs/e56_priorres_local_calibrated_depth.log

echo "DONE E56 local UPH-residual diffusion $(date '+%F %T')" | tee -a results/remote_logs/e56_priorres_local_master.log
