...
for seed in $SEEDS; do
  name="pip_d11_fixed_pixel_gate_d8_seed${seed}"
  ...
  echo "===== D11 START seed=${seed} $(date '+%F %T') ====="
  /root/miniconda3/bin/python eval_pixel_adaptive_gate.py \
    ...
    --conf_mins "0.0" \
    --require_cache
  echo "===== D11 END seed=${seed} $(date '+%F %T') ====="
done
...
rows = []
for p in sorted(base.glob('pip_d11_fixed_pixel_gate_d8_seed*/pixel_gate_summary.json')):
    seed = p.parent.name.split('seed')[-1]
    d = json.load(open(p, 'r', encoding='utf-8'))
    rows.append({
        'seed': seed,
        'val_base': d['val']['base']['rmse']['mean'],
        'val_pixel': d['val']['pixel_gated']['rmse']['mean'],
        'test_base': d['test']['base']['rmse']['mean'],
        'test_pixel': d['test']['pixel_gated']['rmse']['mean'],
        'test_edge_base': d['test']['base']['edge_rmse']['mean'],
        'test_edge_pixel': d['test']['pixel_gated']['edge_rmse']['mean'],
        'test_mae_base': d['test']['base']['mae']['mean'],
        'test_mae_pixel': d['test']['pixel_gated']['mae']['mean'],
...
if rows:
    ...
    summary = {
        'n': len(rows),
        'gate': {'alpha': 0.25, 'edge_th': 0.8, 'delta_min': 0.12, 'conf_min': 0.0},
        'test_base_rmse_mean': mean('test_base'),
        'test_pixel_rmse_mean': mean('test_pixel'),
        'test_base_mae_mean': mean('test_mae_base'),
        'test_pixel_mae_mean': mean('test_mae_pixel'),
        'test_base_edge_rmse_mean': mean('test_edge_base'),
        'test_pixel_edge_rmse_mean': mean('test_edge_pixel'),
        'test_selected_frac_mean': mean('test_selected_frac'),
        'wins': sum(float(r['test_pixel']) < float(r['test_base']) for r in rows),
        'rows': rows,
    }
    out = base / 'pip_d11_fixed_pixel_gate_summary.json'
    json.dump(summary, open(out, 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
    print(json.dumps(summary, ensure_ascii=False))
PY

echo "===== D11 ALL DONE $(date '+%F %T') ====="
