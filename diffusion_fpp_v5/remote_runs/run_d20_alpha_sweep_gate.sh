...
run_alpha_sweep() {
  ...
  echo "===== D20 ALPHA SWEEP ${name} checkpoints=${#checkpoints[@]} $(date '+%F %T') ====="
...
out = base / "pip_d20_alpha_sweep_gate_summary.json"
out.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
print(json.dumps(summary, indent=2, ensure_ascii=False))
PY

echo "===== D20 ALL DONE $(date '+%F %T') ====="
