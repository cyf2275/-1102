...
SAVE=results/fpp960_e57_depthselected_uph_diffusion_ch24_e30
PREFIX=phase_pred_e57_depthselected_uph_ddim20_ftp01
DEPTH_OUT=results/fpp960_e57_depthselected_uph_calibrated_depth

echo "START E57 depth-selected UPH diffusion $(date '+%F %T')" | tee results/remote_logs/e57_depthselected_master.log
...
"$PY" train_fpp_phase_diffusion.py \
  ...
  --save_every 1 \
  --seed 45 \
  2>&1 | tee results/remote_logs/fpp960_e57_depthselected_uph_diffusion_ch24_e30.log

echo "PRECOMPUTE E57 $(date '+%F %T')" | tee -a results/remote_logs/e57_depthselected_master.log
"$PY" precompute_fpp_phase_diffusion_predictions.py \
  ...
  --sample_start_from ftp \
  --sample_start_ratio 0.1 \
  2>&1 | tee results/remote_logs/e57_depthselected_precompute.log

echo "CALIBRATED DEPTH E57 $(date '+%F %T')" | tee -a results/remote_logs/e57_depthselected_master.log
"$PY" eval_phase_calibrated_depth.py \
  ...
  --uph_prior_summary "$PRIOR" \
  2>&1 | tee results/remote_logs/e57_depthselected_calibrated_depth.log

echo "DONE E57 depth-selected UPH diffusion $(date '+%F %T')" | tee -a results/remote_logs/e57_depthselected_master.log
