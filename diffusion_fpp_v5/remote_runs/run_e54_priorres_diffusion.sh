...
SAVE=results/fpp960_e54_uph_priorres_diffusion_ch24_e30
PREFIX=phase_pred_e54_uph_priorres_ddim20_ftp03
DEPTH_OUT=results/fpp960_e54_uph_priorres_calibrated_depth

echo "START E54 prior residual UPH diffusion $(date '+%F %T')" | tee results/remote_logs/e54_priorres_master.log
...
"$PY" train_fpp_phase_diffusion.py \
  ...
  --save_every 1 \
  --seed 42 \
  2>&1 | tee results/remote_logs/fpp960_e54_uph_priorres_diffusion_ch24_e30.log

echo "PRECOMPUTE E54 $(date '+%F %T')" | tee -a results/remote_logs/e54_priorres_master.log
"$PY" precompute_fpp_phase_diffusion_predictions.py \
  ...
  --sample_start_from ftp \
  --sample_start_ratio 0.3 \
  2>&1 | tee results/remote_logs/e54_priorres_precompute.log

echo "CALIBRATED DEPTH E54 $(date '+%F %T')" | tee -a results/remote_logs/e54_priorres_master.log
"$PY" eval_phase_calibrated_depth.py \
  ...
  --uph_prior_summary "$PRIOR" \
  2>&1 | tee results/remote_logs/e54_priorres_calibrated_depth.log

echo "DONE E54 prior residual UPH diffusion $(date '+%F %T')" | tee -a results/remote_logs/e54_priorres_master.log
