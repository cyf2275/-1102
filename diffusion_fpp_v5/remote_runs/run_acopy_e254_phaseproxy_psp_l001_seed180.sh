...
SAVE=results/fpp960_e254_e63ens_psp_phaseproxy_l001_seed180_e35_preload_fast
FUSION=results/e254_d47_seed180_phaseproxy_l001_fusion_rows
GATE_E84=results/e255_e254_e84_phaseproxy_l001_rule
MASTER=results/remote_logs/e254_phaseproxy_psp_l001_seed180_master.log

echo "START E254 teacher-phase proxy PSP lambda=0.01 seed180 $(date '+%F %T')" | tee "$MASTER"
...
if [[ ! -f "$SAVE/evaluation/summary.json" ]]; then
  echo "TRAIN E254 PSP + weak teacher-phase proxy loss seed180 $(date '+%F %T')" | tee -a "$MASTER"
  "$PY" train_fpp_psp_adapter_unet.py \
    ...
    --preload_ram \
    --seed 180 \
    2>&1 | tee results/remote_logs/e254_phaseproxy_psp_l001_seed180_train.log
else
  echo "SKIP E254 train; existing $SAVE/evaluation/summary.json" | tee -a "$MASTER"
fi

echo "FUSE E254 PSP branch with D47 diffusion posterior $(date '+%F %T')" | tee -a "$MASTER"
"$PY" eval_hierarchical_phase_fusion.py \
  ...
  --splits "val test" \
  --require_cache \
  2>&1 | tee results/remote_logs/e254_d47_phaseproxy_l001_fusion.log

echo "APPLY E84/RCPC rule to E254 rows $(date '+%F %T')" | tee -a "$MASTER"
"$PY" select_edge_aware_phase_gate_csv.py \
  ...
  --high_weight 0.6 \
  --save_dir "$GATE_E84" \
  2>&1 | tee results/remote_logs/e255_e254_e84_phaseproxy_l001_rule.log

"$PY" - <<'PY' | tee results/remote_logs/e254_phaseproxy_psp_l001_seed180_summary.txt
import json
from pathlib import Path

paths = {
    "E254_direct_phaseproxy_PSP_l001": Path("results/fpp960_e254_e63ens_psp_phaseproxy_l001_seed180_e35_preload_fast/evaluation/summary.json"),
    "E254_D47_fusion": Path("results/e254_d47_seed180_phaseproxy_l001_fusion_rows/hierarchical_phase_fusion_summary.json"),
    "E255_E84_RCPC": Path("results/e255_e254_e84_phaseproxy_l001_rule/edge_aware_phase_gate_summary.json"),
}
...
Path("results/e254_phaseproxy_psp_l001_seed180_summary.json").write_text(
    json.dumps(out, indent=2, ensure_ascii=False), encoding="utf-8"
)
PY

echo "DONE E254 teacher-phase proxy PSP lambda=0.01 seed180 $(date '+%F %T')" | tee -a "$MASTER"
