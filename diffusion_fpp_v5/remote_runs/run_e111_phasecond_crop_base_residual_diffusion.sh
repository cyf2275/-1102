...
SAVE=results/fpp960_e111_phasecond_crop640_base_residual_ch32_e12
GATE=results/e111_best_hierarchical_physical_gate
MASTER=results/remote_logs/e111_phasecond_crop_base_residual_master.log

echo "START E111 phase-conditioned crop base-residual diffusion $(date '+%F %T')" | tee "$MASTER"
...
"$PY" train_pip_lite.py \
  ...
  --skip_final_test \
  2>&1 | tee results/remote_logs/e111_phasecond_crop_base_residual_train.log

echo "EVAL E111 best checkpoint with hierarchical physical gate $(date '+%F %T')" | tee -a "$MASTER"
"$PY" eval_hierarchical_physical_gate.py \
  ...
  --high_conf_max 0.80 \
  --require_cache \
  2>&1 | tee results/remote_logs/e111_hierarchical_gate_eval.log

echo "DONE E111 $(date '+%F %T')" | tee -a "$MASTER"
