...
SAVE=results/fpp960_e120_e63ens_psp_instrxy_adapter_decoder_seed120_e35_preload_fast
FUSION=results/e120_d47_seed120_phase_fusion_rows
GATE_E89=results/e121_e120_e89_rule
GATE_E84=results/e122_e120_e84_rule
MASTER=results/remote_logs/e120_seed120_phase_depth_fusion_fast_master.log

echo "START E120 seed120 fast phase-depth branch + D47 fusion $(date '+%F %T')" | tee "$MASTER"
...
echo "TRAIN E120 phase-depth adapter seed120 $(date '+%F %T')" | tee -a "$MASTER"
"$PY" train_fpp_psp_adapter_unet.py \
  ...
  --preload_ram \
  --seed 120 \
  2>&1 | tee results/remote_logs/e120_phase_depth_seed120_fast_train.log

echo "FUSE E120 phase-depth branch with D47 depth diffusion posterior $(date '+%F %T')" | tee -a "$MASTER"
"$PY" eval_hierarchical_phase_fusion.py \
  ...
  --splits "val test" \
  --require_cache \
  2>&1 | tee results/remote_logs/e120_d47_phase_fusion.log

echo "APPLY E89 validation-selected ambiguity gate to E120 rows $(date '+%F %T')" | tee -a "$MASTER"
"$PY" select_edge_aware_phase_gate_csv.py \
  ...
  --high_weight 0.6 \
  --save_dir "$GATE_E89" \
  2>&1 | tee results/remote_logs/e121_e120_e89_rule.log

echo "APPLY E84 strict confidence gate to E120 rows $(date '+%F %T')" | tee -a "$MASTER"
"$PY" select_edge_aware_phase_gate_csv.py \
  --val_hier_csv "$FUSION/val_hier_phase_rows.csv" \
  --test_hier_csv "$FUSION/test_hier_phase_rows.csv" \
  --val_fused_csv "$FUSION/val_fused_weight_rows.csv" \
  ...
  --high_weight 0.6 \
  --save_dir "$GATE_E84" \
  2>&1 | tee results/remote_logs/e122_e120_e84_rule.log

"$PY" - <<'PY' | tee results/remote_logs/e120_seed120_summary.txt
import json
from pathlib import Path

paths = {
    "E120_direct_phase_depth": Path("results/fpp960_e120_e63ens_psp_instrxy_adapter_decoder_seed120_e35_preload_fast/evaluation/summary.json"),
    "E120_D47_fusion": Path("results/e120_d47_seed120_phase_fusion_rows/hierarchical_phase_fusion_summary.json"),
    "E121_E89_rule": Path("results/e121_e120_e89_rule/edge_aware_phase_gate_summary.json"),
    "E122_E84_rule": Path("results/e122_e120_e84_rule/edge_aware_phase_gate_summary.json"),
}
...
Path("results/e120_seed120_phase_depth_fusion_summary.json").write_text(
    json.dumps(out, indent=2, ensure_ascii=False), encoding="utf-8"
)
PY

echo "DONE E120 seed120 fast phase-depth branch + D47 fusion $(date '+%F %T')" | tee -a "$MASTER"
