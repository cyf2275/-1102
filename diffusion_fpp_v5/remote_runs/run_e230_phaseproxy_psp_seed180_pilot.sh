...
SAVE=results/fpp960_e230_e63ens_psp_phaseproxy_l005_seed180_e35_preload_fast
FUSION=results/e230_d47_seed180_phaseproxy_fusion_rows
GATE_E84=results/e231_e230_e84_phaseproxy_rule
MASTER=results/remote_logs/e230_phaseproxy_psp_seed180_master.log

echo "START E230 teacher-phase proxy PSP pilot $(date '+%F %T')" | tee "$MASTER"
...
if [[ ! -f "$SAVE/evaluation/summary.json" ]]; then
  echo "TRAIN E230 PSP + teacher-phase proxy loss seed180 $(date '+%F %T')" | tee -a "$MASTER"
  "$PY" train_fpp_psp_adapter_unet.py \
    ...
    --preload_ram \
    --seed 180 \
    2>&1 | tee results/remote_logs/e230_phaseproxy_psp_seed180_train.log
else
  echo "SKIP E230 train; existing $SAVE/evaluation/summary.json" | tee -a "$MASTER"
fi

echo "FUSE E230 PSP branch with D47 diffusion posterior $(date '+%F %T')" | tee -a "$MASTER"
"$PY" eval_hierarchical_phase_fusion.py \
  ...
  --splits "val test" \
  --require_cache \
  2>&1 | tee results/remote_logs/e230_d47_phaseproxy_fusion.log

echo "APPLY E84/RCPC rule to E230 rows $(date '+%F %T')" | tee -a "$MASTER"
"$PY" select_edge_aware_phase_gate_csv.py \
  ...
  --high_weight 0.6 \
  --save_dir "$GATE_E84" \
  2>&1 | tee results/remote_logs/e231_e230_e84_phaseproxy_rule.log

"$PY" - <<'PY' | tee results/remote_logs/e230_phaseproxy_psp_seed180_summary.txt
import json
from pathlib import Path

paths = {
    "E230_direct_phaseproxy_PSP": Path("results/fpp960_e230_e63ens_psp_phaseproxy_l005_seed180_e35_preload_fast/evaluation/summary.json"),
    "E230_D47_fusion": Path("results/e230_d47_seed180_phaseproxy_fusion_rows/hierarchical_phase_fusion_summary.json"),
    "E231_E84_RCPC": Path("results/e231_e230_e84_phaseproxy_rule/edge_aware_phase_gate_summary.json"),
}
...
Path("results/e230_phaseproxy_psp_seed180_summary.json").write_text(
    json.dumps(out, indent=2, ensure_ascii=False), encoding="utf-8"
)
PY

echo "DONE E230 teacher-phase proxy PSP pilot $(date '+%F %T')" | tee -a "$MASTER"
