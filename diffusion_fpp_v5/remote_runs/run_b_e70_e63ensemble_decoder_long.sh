...
SAVE=results/fpp960_e70_e63ens_psp_instrxy_adapter_decoder_bs4_e80_lr3e4
MASTER=results/remote_logs/e70_e63ensemble_decoder_long_master.log

echo "START E70 E63 ensemble phase depth adapter decoder-long $(date '+%F %T')" | tee "$MASTER"
...
"$PY" train_fpp_psp_adapter_unet.py \
  ...
  --train_minimal \
  --seed 70 \
  2>&1 | tee results/remote_logs/fpp960_e70_e63ens_psp_instrxy_adapter_decoder_bs4_e80_lr3e4.log

"$PY" - <<'PY' | tee results/remote_logs/e70_depth_comparison.txt
import json, os

paths = {
    "D47_hierarchical": "results/d47_d31_epoch001_hierarchical_physical_gate/hierarchical_gate_summary.json",
    "E64_E63_adapter_decoder_e25": "results/fpp960_e64_e63ens_psp_instrxy_adapter_decoder_bs4_e25/evaluation/summary.json",
    "E70_E63_adapter_decoder_e80_lr3e4": "results/fpp960_e70_e63ens_psp_instrxy_adapter_decoder_bs4_e80_lr3e4/evaluation/summary.json",
}
...
with open("results/e70_depth_comparison_summary.json", "w", encoding="utf-8") as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
PY

echo "DONE E70 $(date '+%F %T')" | tee -a "$MASTER"
