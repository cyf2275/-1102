...
PHASE_PREFIX=phase_pred_e63_ens_e28_e62_w055_e123_donly
SAVE=results/fpp960_e128_pipfull_donly_fullx0_basestart_ch24_e32
MASTER=results/remote_logs/e128_pipfull_donly_fullx0_master.log

echo "START E128 PIP-Full D_c-only full_x0 base-start diffusion $(date '+%F %T')" | tee "$MASTER"
...
"$PY" train_pip_lite.py \
  ...
  2>&1 | tee results/remote_logs/e128_pipfull_donly_fullx0_train.log

"$PY" eval_hierarchical_physical_gate.py \
  --checkpoint "$SAVE/checkpoints/best.pt" \
  --cache_dir "$BASE_CACHE" \
  --base_prefix "$BASE_PREFIX" \
  --save_dir results/e128_pipfull_donly_fullx0_best_gate_eval \
  --image_h 960 \
  --image_w 960 \
  --ddim_steps 20 \
  --ensemble 1 \
  ...
  --pixel_conf_min 0.0 \
  --require_cache \
  2>&1 | tee results/remote_logs/e128_pipfull_donly_fullx0_gate_eval.log

echo "DONE E128 PIP-Full D_c-only full_x0 base-start diffusion $(date '+%F %T')" | tee -a "$MASTER"
