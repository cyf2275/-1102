...
def gated_summary(rows, selector, candidate_prefix):
...
def selected_subset_summary(rows, selector, candidate_prefix):
    selected = [row for row in rows if selector(row)]
    if not selected:
        return {"n": 0}
    out = {"n": len(selected)}
    for prefix in ("base", candidate_prefix, "diff"):
        out[prefix] = direct_summary(selected, prefix)
    out[f"{candidate_prefix}_wins"] = int(
        sum(row[f"{candidate_prefix}_rmse"] < row["base_rmse"] for row in selected)
    )
    return out
...
def main():
    ...
    parser.add_argument("--candidate_prefix", default="ensemble")
    ...
    candidate = args.candidate_prefix
    result = {
        "fixed_gate": {
        ...
        "val": {
            ...
            candidate: direct_summary(val_rows, candidate),
            "gated": gated_summary(val_rows, selector, candidate),
            "selected_subset": selected_subset_summary(val_rows, selector, candidate),
        },
        "test": {
            "base": direct_summary(test_rows, "base"),
            candidate: direct_summary(test_rows, candidate),
            "gated": gated_summary(test_rows, selector, candidate),
            "selected_subset": selected_subset_summary(test_rows, selector, candidate),
        },
    }
