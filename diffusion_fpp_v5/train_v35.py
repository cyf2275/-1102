"""Train v3.5 phase/edge-conditioned full-height x0 diffusion."""
...
def evaluate_split(diffusion, loader, device, height_scale, split_name, args,
                   out_dir=None, save_images=False):
    ...
    for idx, batch in enumerate(tqdm(loader, desc=f"eval {split_name}")):
        ...
        pred = diffusion.sample_ddim(cond, steps=args.ddim_steps,
                                     ensemble_size=args.ensemble, progress=False)
...
def main():
    ...
    parser.add_argument("--save_dir", default="/root/diffusion_fpp_v5/results/v35_hilbert_dwt")
    ...
    parser.add_argument("--lambda_edge", type=float, default=0.15)
    ...
    model = ConditionalUNet(cond_channels=8, base_ch=args.base_channels,
    ...
    for ep in range(1, args.epochs + 1):
        ...
        total = 0.0
        seen = 0
        for batch in tqdm(loaders["train"], desc=f"v3.5 {ep}/{args.epochs}"):
            cond = batch["cond"].to(device, non_blocking=True)
        ...
        if ep == 1 or ep % args.eval_every == 0:
            ...
            if val_rmse < best:
                ...
                if val_rows:
                    ...
                    cond = first["cond"].to(device)
                    fringe = first["fringe"].to(device)
                    target_raw = first["height_raw"].to(device)
                    pred = diffusion.sample_ddim(cond, steps=args.ddim_steps, ensemble_size=args.ensemble)
                    pred_mm = torch.clamp((pred + 1.0) * 0.5, 0.0, 1.0) * height_scale
                    save_comparison(fringe, target_raw, pred_mm,
                                    save_dir / "visualizations" / f"val_ep{ep:03d}.png",
                                    title=f"v3.5 val RMSE {best:.2f}mm")
        ...
        torch.save({"epoch": ep, "model_state_dict": model.state_dict(), "args": vars(args),
                    "height_scale": height_scale}, save_dir / "checkpoints" / "latest.pt")
