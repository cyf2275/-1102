...
def best_single_feature_gate(rows, feature, rule, candidate_prefix, min_selected=1):
    best = None
    for threshold in threshold_candidates(rows, feature):
        selector = make_selector(feature, rule, threshold)
        summary = summarize(rows, selector, candidate_prefix)
...
def main():
    ...
    selector = make_selector(best["feature"], best["rule"], best["threshold"])
    result = {
        "selected_gate": {
        ...
        "val": {
            ...
            "gated": summarize(val_rows, selector, args.candidate_prefix),
        },
        "test": {
            "base": direct_summary(test_rows, "base"),
            args.candidate_prefix: direct_summary(test_rows, args.candidate_prefix),
            "gated": summarize(test_rows, selector, args.candidate_prefix),
        },
    ...
    print(json.dumps({
        ...
        f"val_{args.candidate_prefix}": result["val"][args.candidate_prefix]["rmse"]["mean"],
        "val_gated": result["val"]["gated"]["rmse"]["mean"],
        "test_base": result["test"]["base"]["rmse"]["mean"],
        f"test_{args.candidate_prefix}": result["test"][args.candidate_prefix]["rmse"]["mean"],
